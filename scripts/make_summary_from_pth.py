...
def plot_dice_per_slice(base_path, patient_id, region):
    ...
    try:
        baseline = pd.read_table(f"{base_path}/baseline_A1T0/{region}/patient{patient_id}/dice", header=None)
        window = pd.read_table(f"{base_path}/window_A1T0/{region}/patient{patient_id}/dice", header=None)
    except:
        return
...
def patient_summary(base_path, window_m, patient_id, region):
    # 各条件におけるdice係数のデータを読み込む。diceがない場合もあるので例外処理
    # pthファイルを読み込む
    try:
        baseline = torch.load(f"{base_path}/w{window_m}_pat{patient_id}_reg{region}_metbaseline_sc.pth")
        window = torch.load(f"{base_path}/w{window_m}_pat{patient_id}_reg{region}_metwindow_sc.pth")
    except:
        return
    # pandasのdfに変換
    df_baseline10 = pd.DataFrame(baseline)
    df_window10 = pd.DataFrame(window)
    # 各dfのnanを削除
...
def plot_and_summary(base_path, window_m):
    ...
    for patient_id in patients:
        for region in regions:
            # 辞書式の結果をリストに追加
            result = patient_summary(base_path, window_m, patient_id, region)
            if result:
    ...
    for region in regions:
        ...
        for result in results[label_dict(region)]:
            if result:
                baseline_results.append(float(result["baseline_A1T0"][0]))
                window_results.append(float(result["window_A1T0"][0]))
        ...
        summary["baseline_A1T0"][label_dict(region)] = [float(x) for x in baseline_results]
        summary["window_A1T0"][label_dict(region)] = [float(x) for x in window_results]
        summary["p-value"][label_dict(region)] = float(p.pvalue)
    
    # 結果の辞書をjson形式で保存
    with open(f"{base_path}/{window_m}_summary.json", "w") as f:
        json.dump(summary, f, indent=4)
            

if __name__ == "__main__":
    for window_m in range(1,6):
        base_path = f"../results/final"
        plot_and_summary(base_path, window_m)
